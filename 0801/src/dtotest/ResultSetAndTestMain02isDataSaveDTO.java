...
public class ResultSetAndTestMain02isDataSaveDTO {
	// dept 테이블의 검색조건에 맞는 데이터를 DTO 클래스를 이용해 List로 저장 후에 출력
	public static void main(String[] args) {
		...
		// Dept테이블에서 읽어온 데이터를 저장하기 위한 자료구조
		...
		try {
			...
			while (rs.next()) { // 읽을 데이터가 있는 지 없는 지를 확인
				...
				DTO dept = new DTO();
				dept.setDeptno(rs.getInt("deptno")); // dept에 데이터 투입 (rs.getInt("db쪽에서 가져올 데이터 이름"));
				dept.setDname(rs.getString("dname"));
				dept.setLoc(rs.getString("loc"));

				list.add(dept); // 읽은 행을 리스트에 저장 -> 하나씩 저장
				con.commit();
			...
			for (DTO d : list) {
				...
				System.out.printf("%20s", d.getDname() );
				System.out.printf("%30s", d.getLoc() );
				System.out.println();
			}
			// else {
